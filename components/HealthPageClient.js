...
function CachedStatusCard({ cachedStatus }) {
  if (!cachedStatus) {
    return (
      <div className="mb-6 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        <p className="text-xs text-slate-500 dark:text-slate-400">
          No scheduled check has run yet (this is written by a GitHub Actions workflow every 6 hours, not by
          this page). Check back later, or use &quot;Check now&quot; below.
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
        Scheduled check (GitHub Actions) — the reliable one
      </p>
      <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">
        Last ran {formatTime(cachedStatus.checkedAt)} —{" "}
        {cachedStatus.healthy ? (
          <span className="font-semibold text-emerald-700 dark:text-emerald-400">no new issues</span>
        ) : (
          <span className="font-semibold text-amber-700 dark:text-amber-400">
            {cachedStatus.errorCount} broken, {cachedStatus.emptyCount} empty, {cachedStatus.orderMismatchCount}{" "}
            order mismatch
          </span>
        )}
        {cachedStatus.knownIssueCount > 0 ? ` (+${cachedStatus.knownIssueCount} known/already-understood)` : ""}
      </p>
    </div>
  );
}

export default function HealthPageClient({ cachedStatus }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="mt-5">
        <CachedStatusCard cachedStatus={cachedStatus} />
      </div>

      <div className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
          Live check (runs from this server)
        </p>
        <p className="mt-1 mb-3 text-xs text-slate-500 dark:text-slate-400">
          Some Indian government sites block or rate-limit requests from cloud-hosting IP ranges (Vercel&apos;s,
          in this case) differently than they treat GitHub Actions&apos; — so a source can show &quot;Broken&quot;
          here even when the scheduled check above says it&apos;s fine. If that happens, trust the scheduled
          check.
        </p>
        <button
          onClick={checkNow}
        ...
        >
          {loading ? "Checking… (up to a minute)" : "Check now"}
        </button>
      </div>
