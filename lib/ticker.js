...
// Keeps the ticker small and self-tidying: once 20 notices are approved,
// approving a 21st silently drops the oldest approval rather than growing
// forever — admin never has to remember to clean up old ones, but can still
// remove any specific one early via the same toggle. 20 (not 10) because
// support.mbbslighthouse.in's Live Updates panel shows the newest 20, while
// mbbslighthouse.in's top ticker only shows the newest 10 of the same feed
// (see those repos' Ticker.js / LiveUpdates.jsx) — both slice from this one
// list, so it needs to hold enough for the larger of the two.
...
function toTickerItem(notice, authorityName, approvedAt) {
  return {
    id: notice.id,
    title: notice.titleEn ?? notice.title,
    link: notice.link ?? null,
    date: notice.date ?? null,
    // NEET Guidance is our own private aggregator, not an official
    // counselling authority — its notices already name the real source in
    // the title itself (e.g. "Karnataka KEA- ..."), so surfacing "NEET
    // Guidance" as the authority label on an embedded feed would
    // misrepresent an unofficial aggregator as an official one.
    authorityName: authorityName === "NEET Guidance" ? null : authorityName,
    approvedAt,
  };
}

// Builds the public ticker payload: admin's manually-approved notices first
// (joined back to their full notice + authority name, newest-approved-first;
// ones that fell off an authority's stored list — each keeps only its latest
// 20, see MAX_NOTICES_PER_AUTHORITY in lib/data.js — are silently skipped
// rather than shown as broken entries), then NEET Guidance's own latest
// notices fill any remaining slots up to MAX_TICKER_ITEMS. So an admin pick
// never fully replaces the auto feed — the two blend, admin's choices always
// ranked first — and an empty approvals list just means every slot is
// auto-filled from NEET Guidance instead of the feed going empty.
export async function getTickerPayload() {
  ...
  const approved = items
    ...
    .map((approval) => {
      ...
      return toTickerItem(notice, authorityName, approval.approvedAt);
    })
    .filter(Boolean);

  const approvedIds = new Set(approved.map((n) => n.id));
  const remainingSlots = MAX_TICKER_ITEMS - approved.length;
  const neetGuidance = authorities.find((a) => a.name === "NEET Guidance");
  const autoFill =
    remainingSlots > 0
      ? (neetGuidance?.notices ?? [])
          .filter((n) => !approvedIds.has(n.id))
          .slice(0, remainingSlots)
          .map((notice) => toTickerItem(notice, "NEET Guidance", notice.detectedAt ?? null))
      : [];

  return [...approved, ...autoFill];
}
