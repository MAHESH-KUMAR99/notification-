...
function fetchInsecure(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(
      url,
      { headers: { "User-Agent": USER_AGENT }, rejectUnauthorized: false, timeout: FETCH_TIMEOUT_MS },
      (res) => {
...
async function fetchWithTimeout(url, { insecureTls = false } = {}) {
  if (insecureTls) return fetchInsecure(url);
...
function inferSortTimestamp(link) {
  if (!link) return null;

  const uploadMatch = link.match(/uploads\/(\d{4})\/(\d{1,2})\//);
  if (uploadMatch) {
    const [, year, month] = uploadMatch;
    return Date.UTC(Number(year), Number(month) - 1, 15);
  }
...
async function extractHtmlList(watcher) {
  const html = await fetchWithTimeout(watcher.url, { insecureTls: watcher.insecureTls });
...
async function extractNmcList(watcher) {
  const body = await fetchWithTimeout(watcher.url, { insecureTls: watcher.insecureTls });
