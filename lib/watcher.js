...
// A bare User-Agent is itself a bot signal to some WAFs — a real Chrome
// navigation always sends this whole set together (Accept, Sec-Fetch-*,
// Client Hints matching the UA string, ...). Several .gov.in sources here
// block/stall requests from GitHub Actions specifically, which is
// consistent with fingerprint-based bot detection rather than a rate limit
// (the same watcher works fine run from a residential IP) — this doesn't
// change the IP, but it removes the "obviously scripted" signal on top of
// it, which is the free thing to try before reaching for infrastructure
// changes like a self-hosted runner.
...
async function fetchOnce(url, insecureTls, timeoutMs) {
...
export async function fetchWithTimeout(
  url,
  { insecureTls = false, timeoutMs = FETCH_TIMEOUT_MS, retries = RETRY_ATTEMPTS } = {}
) {
  let lastError;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      return await fetchOnce(url, insecureTls, timeoutMs);
    } catch (err) {
      lastError = err;
