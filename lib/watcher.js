...
import { DEFAULT_SOURCE_LABEL } from "./sourceLabel.js";
import { getWatchState, setWatchEntry } from "./watchState.js";
import { translateTitle } from "./translate.js";
import { BROWSER_HEADERS } from "./browserHeaders.js";

const WATCHERS_PATH = path.join(process.cwd(), "data", "watchers.json");
const FETCH_TIMEOUT_MS = 15000;
// Base URL of this app's own Vercel deployment — used by `viaProxy` watchers
// to route their fetch through app/api/fetch-proxy/route.js instead of
// hitting the source directly from GitHub Actions. Overridable via env in
// case the production domain ever changes; the literal fallback is this
// project's actual deployed domain, since the scheduled cron script runs
// outside Next.js and has no other way to know it.
const PROXY_BASE_URL = process.env.PROXY_BASE_URL ?? "https://notification-one-mocha.vercel.app";
...
function fetchInsecure(url, timeoutMs) {
  return new Promise((resolve, reject) => {
    const req = https.get(
      url,
      { headers: BROWSER_HEADERS, rejectUnauthorized: false, timeout: timeoutMs },
      (res) => {
...
async function fetchOnce(url, insecureTls, timeoutMs, viaProxy) {
  // Routed through our own Vercel deployment (a different IP range than
  // GitHub Actions) for the handful of sources that block/stall requests
  // specifically from GitHub Actions' CI IP ranges — see PROXY_BASE_URL and
  // app/api/fetch-proxy/route.js. The proxy does its own plain fetch, so
  // `insecureTls` doesn't apply to this branch (none of the proxied hosts
  // need it).
  if (viaProxy) {
    const proxyUrl = `${PROXY_BASE_URL}/api/fetch-proxy?url=${encodeURIComponent(url)}`;
    try {
      return await fetchOnce(proxyUrl, false, timeoutMs, false);
    } catch (proxyErr) {
      // Which network path is currently blocked shifts over time (that's
      // the whole reason viaProxy watchers exist — GitHub Actions' IP range
      // was blocked when they were added, but the blocklists on these
      // .gov.in hosts move independently of us and of each other). Rather
      // than trust whichever path was true when the watcher was configured,
      // fall back to a direct fetch — cheap insurance for the case where
      // it's the *proxy's* current IP that's blocked/unreachable instead.
      try {
        return await fetchOnce(url, insecureTls, timeoutMs, false);
      } catch {
        // Direct failed too — surface the proxy's error, re-tagged so a
        // non-2xx from *our own* proxy (see app/api/fetch-proxy/route.js's
        // 502) isn't mistaken by fetchWithTimeout for a deterministic
        // answer from the origin site the way a direct "HTTP 404" would be,
        // which would otherwise skip the retry budget.
        if (/^HTTP \d+$/.test(proxyErr.message)) throw new Error(`Proxy ${proxyErr.message}`);
        throw proxyErr;
      }
    }
  }
...
// Extra attempts on top of the first, only for network-level failures (DNS/
// TLS/connection resets, timeouts) — several government hosts here
// intermittently refuse or stall connections specifically from GitHub
// Actions' IP ranges (reproduces reliably in the scheduled workflow, not
// when the same watcher is tested from a residential IP), so a short retry
// absorbs that instead of losing a whole 30-minute cycle to it. A real HTTP
// status (404, 403, ...) is deterministic — retrying gets the same answer
// every time — so those fail immediately instead of burning the budget.
const RETRY_ATTEMPTS = 2;
const RETRY_DELAY_MS = 1500;

// `timeoutMs` defaults to the normal 15s used by the scheduled scrape;
// pass a shorter one for callers racing a hard deadline. `retries` defaults
// to RETRY_ATTEMPTS for the same reason — override to 0 for a caller that
// can't afford the extra worst-case latency.
export async function fetchWithTimeout(
  url,
  { insecureTls = false, timeoutMs = FETCH_TIMEOUT_MS, retries = RETRY_ATTEMPTS, viaProxy = false } = {}
) {
  let lastError;
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      return await fetchOnce(url, insecureTls, timeoutMs, viaProxy);
    } catch (err) {
      lastError = err;
...
// Some sources (e.g. Karnataka KEA) have an unrelated unclosed `<!--`
// comment earlier on the page that swallows the actual notice table into
// it as far as a standards-compliant parser (cheerio/htmlparser2, and
// technically browsers too) is concerned — confirmed by counting
// <!--/--> pairs before the table's own markup. `stripBeforeMarker` (a
// regex string) discards everything before its first match, so cheerio
// only ever sees the fragment starting at the real content — outside
// whatever unclosed comment came before it on the full page.
function applyStripBeforeMarker(html, watcher) {
  if (!watcher.stripBeforeMarker) return html;
  const match = html.match(new RegExp(watcher.stripBeforeMarker));
...
function extractNoticeFromNode($, node, watcher) {
  ...
  // Filename-embedded date (auto-generated at upload) is normally more
  // trustworthy than a date found in the title text, which is often a
  // deadline or event date rather than the notice's publish date. Gujarat
  // (medadmgujarat.org) inverts that: its filenames encode a server upload
  // timestamp that can trail the notice's own "dated DD.MM.YYYY" by weeks
  // (seen: a rule dated 30.09.2025 uploaded as .../202511041622204264.pdf,
  // a Nov-2025 upload timestamp), while every title reliably ends with the
  // real date — so `preferTitleDate` swaps the priority for watchers where
  // the filename isn't a trustworthy signal.
  const date = watcher.preferTitleDate
    ? dateFromSelector ??
...
async function extractHtmlList(watcher) {
  const html = await fetchWithTimeout(watcher.url, { insecureTls: watcher.insecureTls, timeoutMs: watcher.timeoutMs, viaProxy: watcher.viaProxy });
...
// DataTables-style sortable date columns often carry the true value in a
// sort attribute (e.g. `data-order="2026-06-06 16:45:06"`) while the
// visible cell text is just the year — e.g. Himachal Pradesh (AMRUHP)'s
// notifications tables. The attribute is the reliable full date; the
// visible text alone isn't usable via parseDateCell below.
function parseDateOrderAttr(value) {
...
async function extractHtmlTable(watcher) {
  const html = await fetchWithTimeout(watcher.url, { insecureTls: watcher.insecureTls, timeoutMs: watcher.timeoutMs, viaProxy: watcher.viaProxy });
  const $ = cheerio.load(applyStripBeforeMarker(html, watcher));
  ...
  rows.each((rowIndex, rowEl) => {
    ...
    const dateCell = watcher.dateCellIndex != null ? cells.eq(watcher.dateCellIndex) : null;

    // Priority: a DataTables `data-order`-style attribute (most authoritative
    // — a full, system-generated date even when the visible cell just shows
    // a year) > an explicit "Published Date" column's own text > a
    // filename-embedded upload date (system-generated, reliable) > a date
    // mentioned in the title text (often a deadline, not the publish date).
    const date =
      (dateCell && watcher.dateAttr ? parseDateOrderAttr(dateCell.attr(watcher.dateAttr)) : null) ??
      (dateCell ? parseDateCell(dateCell.text()) : null) ??
      inferExactDateFromLink(link) ??
      extractEmbeddedDate(title);
...
async function extractNmcList(watcher) {
  const body = await fetchWithTimeout(watcher.url, { insecureTls: watcher.insecureTls, timeoutMs: watcher.timeoutMs, viaProxy: watcher.viaProxy });
...
export async function extractNotices(watcher) {
  ...
  if (watcher.sourceLabel) {
    ...
    // No explicit label (most watchers today — one extractor per
    // authority): still tag a display name, DEFAULT_SOURCE_LABEL, so
    // DetailPanel's tabs read "Direct from Website" instead of a blank/
    // internal key the moment a second, explicitly-labelled watcher is
    // added for the same authority later. Deliberately NOT folded into the
    // `id` (unlike the branch above) — these ids are already the dedup key
    // stored in watch-state.json's seenIds; prefixing them here would make
    // every already-seen notice look brand new on the next scheduled run.
    notices = notices.map((n) => ({ ...n, source: DEFAULT_SOURCE_LABEL }));
  }
