...
async function fetchOnce(url, insecureTls, timeoutMs, viaProxy) {
  ...
  if (viaProxy) {
    ...
    try {
      ...
      // A non-2xx here is *our own* proxy reporting it couldn't reach the
      // target (see app/api/fetch-proxy/route.js's 502) — that's a network
      // condition worth retrying, not a deterministic answer from the
      // origin site the way a direct-fetch "HTTP 404" would be. Re-tag it
      // so fetchWithTimeout's deterministic-status short-circuit doesn't
      // swallow the retry budget.
      if (/^HTTP \d+$/.test(err.message)) throw new Error(`Proxy ${err.message}`);
      throw err;
    }
  }
